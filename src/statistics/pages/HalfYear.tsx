        ...
        <article className="card">
          ...
          {dash.byModel.rows.length === 0 ? (
            ...
            <>
              <table className="tbl" style={{ marginTop: 10 }}>
                <thead>
                  <tr>
                    ...
                    <th className="r">每 $1 API 扣的額度</th>
                  </tr>
                </thead>
                <tbody>
                        ...
                        <td className="r" style={{ width: 140 }}>
                          ...
                          {xc?.multiplier != null && (
                            <div className="mult-bar">
                              <div className={xc.multiplier > 1.15 ? "hot" : ""} style={{ width: `${Math.min(100, xc.multiplier * 50)}%` }} />
                            </div>
                          )}
                        </td>
                      </tr>,
                      ...rows.map((m) => (
                        <tr key={m.model}>
                          ...
                          <td className="r" style={{ width: 140 }}>
                            ...
                            {m.multiplier != null && (
                              <div className="mult-bar">
                                <div className={m.multiplier > 1.15 ? "hot" : ""} style={{ width: `${Math.min(100, m.multiplier * 50)}%` }} />
                              </div>
                            )}
                          </td>
                        </tr>
              ...
              <p className="foot-note zh" style={{ marginTop: 8 }}>
                $/1% 越低，代表同樣 1% 的額度能換到的 API 呼叫量越少——這個模型比較「傷額度」。倍率是跟整體平均比；量條最高顯示到 2.0×。
                「校準中」表示樣本還不夠，數字先不要當真。家族列是同家族各版本合併計算的綜合費率；旁邊的符號顯示新舊兩種算法的結果是否吻合。
              </p>
              <How>
                倍率＝期間修剪匯率 ÷ 該模型匯率。回歸：每段區間 Δ% ＝ Σ 各模型 API$ × 係數（加權 NNLS：係數 ≥0、無截距、權重 1/usd、與匯率同一套修剪），
