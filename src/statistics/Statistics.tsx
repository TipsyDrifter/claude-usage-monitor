...
import { seatLabel, ALL_SEATS, ALL_SEATS_LABEL } from "@/lib/seatLabel";
          ...
          {seats.length > 0 && (
            <label className="zh" style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6, fontSize: 11, color: "var(--h5-ink2)" }}>
              ...
              >
                ...
                {/* v1.8.11（D89，主人）：全量視角——趨勢／歷史把所有帳號合起來算 */}
                <option value={ALL_SEATS}>{ALL_SEATS_LABEL}</option>
              </select>
              {viewSeatId === ALL_SEATS ? (
                <span style={{ fontSize: 10.5, color: "var(--h5-ink3)" }}>趨勢／歷史合計；今日頁仍看目前帳號</span>
              ) : viewSeatId != null ? (
