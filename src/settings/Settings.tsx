  ...
  return (
    <div className="h5 min-h-screen p-6" style={{ background: "var(--h5-bg)" }}>
      ...
      >
            ...
            <div className="grid grid-cols-3 gap-2">
              {FACE_THEMES.map((k) => {
                ...
                return (
                  ...
                  >
                    {/* v1.8.8（主人 2026-09-25）：W1／W4／W5 是原型編號，對使用者沒意義，只留名字 */}
        ...
        <Card title="採集節奏" fields={["general.pollIntervalMinutes", "general.probeMinMinutes", "general.probeHotMinutes", "general.idleHeartbeatMinutes"]}>
          <SliderRow
            label="背景重讀間隔"
            description="就算沒有任何門鈴響，最久每隔這麼久也會重讀一次本機的紀錄、把畫面上的時間更新。這只是保底，不等於每次都去查額度。"
          ...
          />
          {/* v1.8.9（D88，主人）：查額度的下限開放調整；以前寫死 10 分快取過期＋3 分地板 */}
          <SliderRow
            label="查額度最少間隔"
            description="有在用 Claude（本機看到新紀錄、或門鈴響）時，距離上一次查額度超過這麼久才再查一次。查一次不花額度，但會在背景跑一次 Claude Code（約 10 秒）。"
            value={settings.general.probeMinMinutes ?? 10}
            min={1}
            max={30}
            step={1}
            unit="分鐘"
            onChange={(v) =>
              updateSettings("general", { probeMinMinutes: v })
            }
          />
          {/* v1.8.10（主人）：熱態間隔也開放；門檻跟「告警」卡的警戒值連動，不再寫死 90 */}
          <SliderRow
            label="接近警戒時的查額度間隔"
            description={`5h 額度到警戒值（現在設 ${settings.notifications.wallPct}%）以上時，改用這個更短的間隔，最快 1 分鐘。`}
            value={settings.general.probeHotMinutes ?? 2}
            min={1}
            max={10}
            step={1}
            unit="分鐘"
            onChange={(v) =>
              updateSettings("general", { probeHotMinutes: v })
            }
          />
          <SliderRow
            label="閒置心跳"
            description="一段時間沒偵測到你在用 Claude，就暫停查額度（出錯也會先等一下）。暫停期間最久隔這麼久還是會查一次，免得數字停在舊值（例如沒裝瀏覽器擴充、只在網頁上用的時候）。設 0 就閒置時完全不查。"
...
function WebhookBlock() {
  ...
  // v1.8.7（D86 蟲 5）：門鈴按了但密碼牌對不上，這裡是能動手修的地方，所以在這裡提醒。
  const [unauth, setUnauth] = useState<{ source: string; count: number; last: string | null }[]>([]);
  const usageStamp = useStore((s) => s.usage.lastSuccessAt);
  useEffect(() => {
    let alive = true;
    cmd
      .getDataHealth()
      .then((h) => {
        if (alive) setUnauth((h?.doorbellUnauthorized ?? []).filter((u: { count: number }) => u.count > 0));
      })
      .catch(() => {
        /* 非 Tauri／demo：沒有健康度可看 */
      });
    return () => {
      alive = false;
    };
  }, [usageStamp, installState]);
  ...
  return (
    <div className="border-t border-slate-100 pt-3 mt-1 space-y-3">
      ...
      {unauth.map((u) => (
        <div key={u.source} className="rounded-md border border-amber-200 bg-amber-50 px-2.5 py-2 text-[10.5px] leading-relaxed text-amber-800">
          {u.source === "claude-code-hook"
            ? <>Claude Code hook 按了 <b>{u.count}</b> 次門鈴但密碼牌對不上。這把牌 App 啟動時會自動換新；現在按一次「一鍵安裝 Claude Code Hook」也可以立刻修好。</>
            : u.source === "extension" || u.source === "extension-test"
            ? <>瀏覽器擴充按了 <b>{u.count}</b> 次門鈴但密碼牌對不上（App 換過牌）。複製下面的密碼牌，重新貼進擴充的視窗。</>
            : <>來源不明的門鈴按了 <b>{u.count}</b> 次但密碼牌對不上（可能是舊版裝的 hook）。按一次「一鍵安裝 Claude Code Hook」會用現在的牌重寫。</>}
        </div>
      ))}
