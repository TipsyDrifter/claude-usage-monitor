...
export const useStore = create<Store>((set, get) => ({
  ...
  loadSeats: async () => {
    ...
    try {
      ...
      if (view != null && view === r.currentSeatId) {
        ...
        // v1.8.11：全量視角沒有「最後已知值」可抓（那是單一座位的東西）
      } else if (view != null) {
  ...
  setViewSeat: async (id) => {
    ...
    // v1.8.11（D89）：全量視角（不分帳號）——趨勢頁／歷史頁把所有座位合起來算；
    // 沒有單一座位的「最後已知值」，即時面照舊顯示目前帳號。
    if (next === "*") {
      set({ viewSeatId: "*", seatSnapshot: null });
      return;
    }
    if (get().demoMode) {
  ...
  init: async () => {
    ...
    // v1.8.7（D86 蟲 2）：3 秒還沒拿到就別放棄——開機自啟時磁碟慢、或 IPC 在啟動頭一秒
    // 失敗，以前這個 store 就一直抱著預設值（主題 w5），懸浮窗穿錯臉直到下一次廣播。
    // 後端 get_settings 現在會等載入完，這裡再多一層：每 2 秒問一次、最多一分鐘。
    if (!get().demoMode && !get().settingsLoaded) {
      void (async () => {
        for (let i = 0; i < 30 && !get().settingsLoaded; i++) {
          await new Promise((r) => setTimeout(r, 2000));
          if (get().settingsLoaded) break;
          try {
