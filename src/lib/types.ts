...
export interface GeneralSettings {
  ...
  /** v1.8.9（D88）：查額度最少間隔（分鐘）——有在用時，距上次查詢超過這麼久才再查；預設 10。 */
  probeMinMinutes: number;
  /** v1.8.10：5h 額度到警戒值以上時改用的更短間隔（分鐘）；預設 2。 */
  probeHotMinutes: number;
...
export const DEFAULT_SETTINGS: AppSettings = {
  general: {
    ...
    autoStart: true, // v1.8.4：新安裝預設開（App 要常駐帳本才會累積）
