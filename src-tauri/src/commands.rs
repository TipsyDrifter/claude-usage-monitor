...
/// v1.8.7（D86 蟲 2）：等後端真的從磁碟載完再回答。webview 比 `setup` 早起，
/// 以前這裡會發 `AppSettings::default()`（主題 w5、密碼牌空）給早到的視窗，靠前端
/// 3 秒重試＋一次廣播補救；開機自啟時磁碟慢就補不到，懸浮窗穿回預設臉。
/// 等超過 1 秒就在 collector.log 留一行，日後對帳用。
#[tauri::command]
pub async fn get_settings(
    ...
    if !state.is_loaded() {
        ...
        let ok = state
            .wait_loaded(std::time::Duration::from_secs(20))
            .await;
        let waited_ms = started.elapsed().as_millis();
        if waited_ms >= 1000 || !ok {
...
pub async fn regenerate_webhook_token(app: AppHandle) -> Result<String, String> {
    ...
    // v1.8.7（D86 蟲 5）：換了密碼牌，已裝的 Stop hook 手上還是舊牌——以前從這一刻起
    // 每次 Claude Code 按門鈴都是 401，沒人知道。已裝就順手換牌。
    crate::hook_installer::repair_if_installed(&app, &new_token, s.webhook.port).await;
    Ok(new_token)
}
