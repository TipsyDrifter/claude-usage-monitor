...
impl Ledger {
    ...
    fn resolve_seat_sync(conn: &Connection, seat: Option<&str>) -> Result<String> {
        if let Some(s) = seat.filter(|s| !s.is_empty()) {
    ...
    fn dashboard_sync(
        ...
        let selected = seats
            ...
            .or(seats.first())
            .cloned();
        let Some((seat_id, seat_acct, seat_org, _, desktop_n, seat_email, seat_org_name)) = selected else {
        ...
        let data_since: Option<String> = conn
            .query_row(
                "SELECT MIN(fetched_at) FROM truth_samples \
                 WHERE seat_id = ?1 AND source IN ('probe','cli-cache')",
                params![seat_id],
        ...
        let iv5 = Self::pair_intervals(&conn, &seat_id, "'session','five_hour'", &since, entrypoint)?;
        let iv7 = Self::pair_intervals(&conn, &seat_id, "'weekly_all','seven_day'", &since, entrypoint)?;
            ...
            .filter_map(|f| {
                ...
                Some(serde_json::json!({
                    ...
                    "multiplier": match (period_trim, r.rate) {
                        (Some(p), Some(rate)) if r.status == "ok" => Some(p / rate),
                        _ => None,
                    },
                    "agree": agree,
                }))
        ...
        mark("reconcile");
        // ---- daily USD (period, entrypoint-filtered) + walls + gaps ----
        ...
        let mut daily: Vec<serde_json::Value> = Vec::new();
        {
            let mut stmt = conn.prepare(
                "SELECT substr(ts,1,10), model, COALESCE(speed,''), COUNT(*), \
                 ...
                 FROM usage_events WHERE ts >= ?1 AND entrypoint = ?2 \
                 GROUP BY 1, 2, 3 ORDER BY 1",
            )?;
            let mut per_day: std::collections::BTreeMap<String, f64> = Default::default();
            for row in stmt.query_map(params![since60, entrypoint], |r| {
                Ok((
                    r.get::<_, String>(0)?,
                    r.get::<_, String>(1)?,
                    r.get::<_, String>(2)?,
        ...
        {
            let mut stmt = conn.prepare(
                "SELECT ts, rate_limit_type FROM anchors WHERE ts >= ?1 ORDER BY ts",
            )?;
        ...
        let mut saturated_days: Vec<String> = Vec::new();
        {
            let mut stmt = conn.prepare(
                "SELECT DISTINCT substr(fetched_at,1,10) FROM truth_samples \
                 WHERE seat_id = ?1 AND percent >= 100 AND fetched_at >= ?2 ORDER BY 1",
            )?;
        ...
        mark("daily");
        // ---- attribution (machine-wide, period, by entrypoint) ----
        ...
        let mut attribution: Vec<serde_json::Value> = Vec::new();
        {
            let mut stmt = conn.prepare(
                "SELECT COALESCE(entrypoint,'unknown'), SUM(output_tokens) \
                 FROM usage_events WHERE ts >= ?1 GROUP BY 1 ORDER BY 2 DESC",
            )?;
            for row in stmt.query_map(params![since30], |r| {
                Ok(serde_json::json!({
                    "entrypoint": r.get::<_, String>(0)?,
...
impl Ledger {
    ...
    fn history_sync(
        ...
        let Some((seat_id, seat_acct, seat_org, _)) = selected else {
            return Ok(serde_json::json!({ "seats": [], "empty": true }));
        };
        ...
        let (rows, windows, anchors) =
            Self::windows_sync(&conn, &seat_id, kind, &from_utc, &to_utc, keep_points)?;

        // 三條額度在期間內的峰值（脈絡列：看 7d 時也知道 5h／Fable 到哪）
        let peak_of = |k: HistKind| -> Result<Option<f64>> {
            Ok(conn.query_row(
                &format!(
                    "SELECT MAX(percent) FROM truth_samples WHERE seat_id = ?1 \
    ...
    pub async fn export_evidence(
        ...
        let intervals: Vec<Interval> = {
            let conn = self.conn.lock().await;
            let (_, selected) = Self::select_seat_sync(&conn, seat_account)?;
        ...
        let fields = serde_json::json!({
            ...
            "byModel": "分模型倍率（v1.6 回歸法）：rows[] 每個模型 id 一列——rate＝$/1%（加權 NNLS 係數的倒數，修剪後區間、w=1/usd、無截距）、ci＝bootstrap 95%（上界 null＝碰到 β→0 無上界）、nSeg 有花錢的段數、zeroShare bootstrap 卡 0 的比例、status ok/calibrating/collinear、multiplier＝期間修剪匯率÷rate（>1＝每 API $1 扣的額度比平均多，只有 ok 才給）；families[] 家族層級的純區間法 vs 回歸交叉驗證（agree）",
...
fn evidence_markdown(dash: &serde_json::Value, days: u32, n_intervals: usize, app_version: &str, generated_at: &str) -> String {
    ...
    if let Some(rows) = dash["byModel"]["rows"].as_array().filter(|a| !a.is_empty()) {
        ...
        let _ = writeln!(s, "| 模型 | $/1% | 95% CI | 有花錢的段數 | 狀態 | 每 API $1 扣的額度（vs 平均） |\n|---|---|---|---|---|---|");
